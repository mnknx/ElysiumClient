...
void CMenus::RenderServerbrowserServerList(CUIRect View, bool &WasListboxItemActivated)
{
	...
	{
		Headers.Draw(ElysiumUi::Theme().m_BgSecondary, IGraphics::CORNER_T, 5.0f);
		Headers.VSplitRight(s_ListBox.ScrollbarWidthMax(), &Headers, nullptr);
		View.Draw(ElysiumUi::Theme().m_BgPrimary.WithMultipliedAlpha(0.35f), IGraphics::CORNER_NONE, 0.0f);
	}
	else
	{
	...
	for(int i = 0; i < NumServers; i++)
	{
		...
		for(const auto &Col : s_aCols)
		{
			...
			else if(Id == COL_PING)
			{
				...
				if(g_Config.m_ClElysiumUi && !pItem->m_LatencyIsEstimated)
				{
					// Elysium: accessible signal bars (shape) + colored number.
					CUIRect Bars, Number;
					Button.VSplitLeft(14.0f, &Bars, &Number);
					ElysiumUi::DrawPingIndicator(Bars, pItem->m_Latency);
					TextRender()->TextColor(ElysiumUi::PingColor(pItem->m_Latency));
					Ui()->DoLabelStreamed(*pUiElement->Rect(UI_ELEM_PING), &Number, aTemp, FontSize, TEXTALIGN_MR);
					TextRender()->TextColor(TextRender()->DefaultTextColor());
				}
				else
				{
...
void CMenus::RenderServerbrowserInfo(CUIRect View)
{
	...
	if(pSelectedServer)
	{
		...
		if(g_Config.m_ClElysiumUi && !pSelectedServer->m_LatencyIsEstimated)
		{
			PingLabelProps.SetColor(ElysiumUi::PingColor(pSelectedServer->m_Latency));
		}
		else if(g_Config.m_UiColorizePing)
		{
		...
		if(g_Config.m_ClElysiumUi && !pSelectedServer->m_LatencyIsEstimated)
		{
			// signal bars next to the ping value
			CUIRect Bars, PingText;
			Row.VSplitLeft(16.0f, &Bars, &PingText);
			ElysiumUi::DrawPingIndicator(Bars, pSelectedServer->m_Latency);
			Ui()->DoLabel(&PingText, aPingLabel, FontSize, TEXTALIGN_ML, PingLabelProps);
		}
		else
		{
