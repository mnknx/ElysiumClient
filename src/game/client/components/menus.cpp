...
int CMenus::DoButton_Menu(CButtonContainer *pButtonContainer, const char *pText, int Checked, const CUIRect *pRect, const unsigned Flags, const char *pImageName, int Corners, float Rounding, float FontFactor, ColorRGBA Color)
{
	...
	if(g_Config.m_ClElysiumUi)
	{
		...
		// Only retint the default menu-button color; explicit caller colors are kept.
		const bool IsDefaultColor = Color.r == 1.0f && Color.g == 1.0f && Color.b == 1.0f && Color.a == 0.5f;
		if(Checked)
			Color = Theme.m_Selected;
		else if(IsDefaultColor)
			Color = Theme.m_CardBg;
	}
	else if(Checked)
...
// ---------------------------------------------------------------------------
// Elysium UI main menu: top bar (logo/version/fps) + left navigation + content
// + bottom status bar. Only used when g_Config.m_ClElysiumUi is enabled.
// ---------------------------------------------------------------------------

int CMenus::DoElysiumNavButton(CButtonContainer *pButtonContainer, const char *pIcon, const char *pText, bool Active, const CUIRect *pRect)
{
	const ElysiumUi::STheme &Theme = ElysiumUi::Theme();
	const bool Hovered = Ui()->HotItem() == pButtonContainer;

	if(Active)
		pRect->Draw(Theme.m_Selected, IGraphics::CORNER_ALL, Theme.m_Rounding);
	else if(Hovered)
		pRect->Draw(Theme.m_Hover, IGraphics::CORNER_ALL, Theme.m_Rounding);

	// clear "selected" indicator: accent bar on the left edge
	if(Active)
	{
		CUIRect Accent;
		pRect->VSplitLeft(3.0f, &Accent, nullptr);
		Accent.Draw(Theme.m_AccentPrimary, IGraphics::CORNER_L, 1.5f);
	}

	CUIRect Icon, Label;
	pRect->VSplitLeft(34.0f, &Icon, &Label);

	// icon (icon font)
	TextRender()->SetFontPreset(EFontPreset::ICON_FONT);
	TextRender()->SetRenderFlags(ETextRenderFlags::TEXT_RENDER_FLAG_ONLY_ADVANCE_WIDTH | ETextRenderFlags::TEXT_RENDER_FLAG_NO_X_BEARING | ETextRenderFlags::TEXT_RENDER_FLAG_NO_Y_BEARING | ETextRenderFlags::TEXT_RENDER_FLAG_NO_PIXEL_ALIGNMENT | ETextRenderFlags::TEXT_RENDER_FLAG_NO_OVERSIZE);
	TextRender()->TextColor(Active ? Theme.m_AccentPrimary : Theme.m_TextSecondary);
	Ui()->DoLabel(&Icon, pIcon, 14.0f, TEXTALIGN_MC);
	TextRender()->SetRenderFlags(0);
	TextRender()->SetFontPreset(EFontPreset::DEFAULT_FONT);

	// label (normal font)
	TextRender()->TextColor(Active ? Theme.m_TextPrimary : Theme.m_TextSecondary);
	Ui()->DoLabel(&Label, pText, Theme.m_FontSizeNormal, TEXTALIGN_ML);
	TextRender()->TextColor(TextRender()->DefaultTextColor());

	return Ui()->DoButtonLogic(pButtonContainer, 0, pRect, BUTTONFLAG_LEFT);
}

void CMenus::RenderElysiumTopBar(CUIRect Bar)
{
	const ElysiumUi::STheme &Theme = ElysiumUi::Theme();
	ElysiumUi::DrawPanel(Bar);
	Bar.VMargin(14.0f, &Bar);

	CUIRect Left, Right;
	Bar.VSplitLeft(Bar.w * 0.5f, &Left, &Right);

	// Logo + version (left)
	CUIRect Logo, Version;
	Left.VSplitLeft(150.0f, &Logo, &Version);
	TextRender()->TextColor(Theme.m_AccentPrimary);
	Ui()->DoLabel(&Logo, "ELYSIUM", Theme.m_FontSizeTitle, TEXTALIGN_ML);
	TextRender()->TextColor(Theme.m_TextSecondary);
	Ui()->DoLabel(&Version, "v" GAME_RELEASE_VERSION, Theme.m_FontSizeSmall, TEXTALIGN_ML);
	TextRender()->TextColor(TextRender()->DefaultTextColor());

	// FPS + connection (right)
	char aBuf[64];
	const int Fps = round_to_int(1.0f / Client()->FrameTimeAverage());
	str_format(aBuf, sizeof(aBuf), "%d FPS", Fps);
	TextRender()->TextColor(Theme.m_TextSecondary);
	Ui()->DoLabel(&Right, aBuf, Theme.m_FontSizeNormal, TEXTALIGN_MR);
	TextRender()->TextColor(TextRender()->DefaultTextColor());
}

void CMenus::RenderElysiumStatusBar(CUIRect Bar)
{
	const ElysiumUi::STheme &Theme = ElysiumUi::Theme();
	ElysiumUi::DrawPanel(Bar);
	Bar.VMargin(14.0f, &Bar);
	...
	CUIRect Left, Right;
	Bar.VSplitMid(&Left, &Right, 10.0f);

	const bool Online = Client()->State() == IClient::STATE_ONLINE;

	TextRender()->TextColor(Theme.m_TextSecondary);
	Ui()->DoLabel(&Left, "Elysium Client — Development Build", Theme.m_FontSizeSmall, TEXTALIGN_ML);

	// status dot + text on the right
	CUIRect Dot, Text;
	Right.VSplitRight(90.0f, &Text, &Dot);
	Ui()->DoLabel(&Text, Online ? Localize("Connected") : Localize("Not connected"), Theme.m_FontSizeSmall, TEXTALIGN_MR);
	CUIRect DotShape;
	Dot.VSplitLeft(10.0f, nullptr, &DotShape);
	DotShape.HMargin((DotShape.h - 8.0f) / 2.0f, &DotShape);
	DotShape.VSplitLeft(8.0f, &DotShape, nullptr);
	DotShape.Draw(Online ? Theme.m_Success : Theme.m_Warning, IGraphics::CORNER_ALL, 4.0f);
	TextRender()->TextColor(TextRender()->DefaultTextColor());
}

int CMenus::RenderElysiumNav(CUIRect Nav, IClient::EClientState ClientState)
{
	ElysiumUi::DrawPanel(Nav);
	Nav.Margin(8.0f, &Nav);

	const int ActivePage = m_MenuPage;
	int NewPage = -1;
	...
	Nav.HSplitTop(ButtonHeight, &Button, &Nav);
	static CButtonContainer s_HomeButton;
	if(DoElysiumNavButton(&s_HomeButton, FontIcon::HOUSE, Localize("Main menu"), false, &Button))
		m_ShowStart = true;
	Nav.HSplitTop(Gap, nullptr, &Nav);

	Nav.HSplitTop(ButtonHeight, &Button, &Nav);
	static CButtonContainer s_NewsButton;
	if(DoElysiumNavButton(&s_NewsButton, FontIcon::NEWSPAPER, Localize("News"), ActivePage == PAGE_NEWS, &Button))
		NewPage = PAGE_NEWS;
	Nav.HSplitTop(Gap, nullptr, &Nav);

	Nav.HSplitTop(ButtonHeight, &Button, &Nav);
	static CButtonContainer s_InternetButton;
	if(DoElysiumNavButton(&s_InternetButton, FontIcon::EARTH_AMERICAS, Localize("Internet"), ActivePage == PAGE_INTERNET, &Button))
		NewPage = PAGE_INTERNET;
	Nav.HSplitTop(Gap, nullptr, &Nav);

	Nav.HSplitTop(ButtonHeight, &Button, &Nav);
	static CButtonContainer s_LanButton;
	if(DoElysiumNavButton(&s_LanButton, FontIcon::NETWORK_WIRED, Localize("LAN"), ActivePage == PAGE_LAN, &Button))
		NewPage = PAGE_LAN;
	Nav.HSplitTop(Gap, nullptr, &Nav);

	Nav.HSplitTop(ButtonHeight, &Button, &Nav);
	static CButtonContainer s_FavoritesButton;
	if(DoElysiumNavButton(&s_FavoritesButton, FontIcon::STAR, Localize("Favorites"), ActivePage == PAGE_FAVORITES, &Button))
		NewPage = PAGE_FAVORITES;
	Nav.HSplitTop(Gap, nullptr, &Nav);

	// favorite community tabs (same behavior as the classic menubar)
	size_t FavoriteCommunityIndex = 0;
	static CButtonContainer s_aFavoriteCommunityButtons[5];
	for(const CCommunity *pCommunity : ServerBrowser()->FavoriteCommunities())
	{
		if(Nav.h < ButtonHeight)
			break;
		Nav.HSplitTop(ButtonHeight, &Button, &Nav);
		const int Page = PAGE_FAVORITE_COMMUNITY_1 + FavoriteCommunityIndex;
		if(DoElysiumNavButton(&s_aFavoriteCommunityButtons[FavoriteCommunityIndex], FontIcon::ELLIPSIS, pCommunity->Name(), ActivePage == Page, &Button))
			NewPage = Page;
		Nav.HSplitTop(Gap, nullptr, &Nav);
		++FavoriteCommunityIndex;
		if(FavoriteCommunityIndex >= std::size(s_aFavoriteCommunityButtons))
	...
	if(DoElysiumNavButton(&s_DemosButton, FontIcon::CLAPPERBOARD, Localize("Demos"), ActivePage == PAGE_DEMOS, &Button))
		NewPage = PAGE_DEMOS;

	// bottom group: settings, editor, quit (pinned to the bottom)
	CUIRect BottomButton;
	Nav.HSplitBottom(ButtonHeight, &Nav, &BottomButton);
	static CButtonContainer s_QuitButton;
	if(DoElysiumNavButton(&s_QuitButton, FontIcon::POWER_OFF, Localize("Quit"), false, &BottomButton))
	{
		if(GameClient()->Editor()->HasUnsavedData() || (GameClient()->CurrentRaceTime() / 60 >= g_Config.m_ClConfirmQuitTime && g_Config.m_ClConfirmQuitTime >= 0) || m_MenusIngameTouchControls.UnsavedChanges() || GameClient()->m_TouchControls.HasEditingChanges())
			m_Popup = POPUP_QUIT;
		else
	...
	Nav.HSplitBottom(Gap, &Nav, nullptr);

	Nav.HSplitBottom(ButtonHeight, &Nav, &BottomButton);
	static CButtonContainer s_EditorButton;
	if(DoElysiumNavButton(&s_EditorButton, FontIcon::PEN_TO_SQUARE, Localize("Editor"), false, &BottomButton))
		g_Config.m_ClEditor = 1;
	Nav.HSplitBottom(Gap, &Nav, nullptr);

	Nav.HSplitBottom(ButtonHeight, &Nav, &BottomButton);
	static CButtonContainer s_SettingsButton;
	if(DoElysiumNavButton(&s_SettingsButton, FontIcon::GEAR, Localize("Settings"), ActivePage == PAGE_SETTINGS, &BottomButton))
		NewPage = PAGE_SETTINGS;

	return NewPage;
}

void CMenus::RenderElysiumMainMenu(CUIRect Screen, IClient::EClientState ClientState)
{
	CUIRect TopBar, Body, StatusBar, Nav, Content;

	Screen.HSplitTop(50.0f, &TopBar, &Body);
	Body.HSplitTop(8.0f, nullptr, &Body);
	Body.HSplitBottom(26.0f, &Body, &StatusBar);
	Body.HSplitBottom(8.0f, &Body, nullptr);
	Body.VSplitLeft(170.0f, &Nav, &Content);
	Content.VSplitLeft(10.0f, nullptr, &Content);

	RenderElysiumTopBar(TopBar);
	const int NewPage = RenderElysiumNav(Nav, ClientState);
	RenderElysiumStatusBar(StatusBar);

	// content: reuse the existing page renderers unchanged
	if(m_MenuPage == PAGE_NEWS)
		RenderNews(Content);
	else if(m_MenuPage >= PAGE_INTERNET && m_MenuPage <= PAGE_FAVORITE_COMMUNITY_5)
		RenderServerbrowser(Content);
	else if(m_MenuPage == PAGE_DEMOS)
		RenderDemoBrowser(Content);
...
int CMenus::RenderElysiumInGameNav(CUIRect Nav)
{
	ElysiumUi::DrawPanel(Nav);
	Nav.Margin(8.0f, &Nav);

	const int ActivePage = m_GamePage;
	int NewPage = -1;
	const float ButtonHeight = 34.0f;
	const float Gap = 4.0f;
	CUIRect Button;
	...
	Nav.HSplitTop(ButtonHeight, &Button, &Nav);
	static CButtonContainer s_GameButton;
	if(DoElysiumNavButton(&s_GameButton, FontIcon::PLAY, Localize("Game"), ActivePage == PAGE_GAME, &Button))
		NewPage = PAGE_GAME;
	Nav.HSplitTop(Gap, nullptr, &Nav);

	Nav.HSplitTop(ButtonHeight, &Button, &Nav);
	static CButtonContainer s_PlayersButton;
	if(DoElysiumNavButton(&s_PlayersButton, FontIcon::USER, Localize("Players"), ActivePage == PAGE_PLAYERS, &Button))
		NewPage = PAGE_PLAYERS;
	Nav.HSplitTop(Gap, nullptr, &Nav);

	Nav.HSplitTop(ButtonHeight, &Button, &Nav);
	static CButtonContainer s_ServerInfoButton;
	if(DoElysiumNavButton(&s_ServerInfoButton, FontIcon::INFO, Localize("Server info"), ActivePage == PAGE_SERVER_INFO, &Button))
		NewPage = PAGE_SERVER_INFO;
	Nav.HSplitTop(Gap, nullptr, &Nav);

	Nav.HSplitTop(ButtonHeight, &Button, &Nav);
	static CButtonContainer s_NetworkButton;
	if(DoElysiumNavButton(&s_NetworkButton, FontIcon::EARTH_AMERICAS, Localize("Browser"), ActivePage == PAGE_NETWORK, &Button))
		NewPage = PAGE_NETWORK;
	Nav.HSplitTop(Gap, nullptr, &Nav);
	...
	if(GameClient()->m_GameInfo.m_Race)
	{
		Nav.HSplitTop(ButtonHeight, &Button, &Nav);
		static CButtonContainer s_GhostButton;
		if(DoElysiumNavButton(&s_GhostButton, FontIcon::FLAG_CHECKERED, Localize("Ghost"), ActivePage == PAGE_GHOST, &Button))
			NewPage = PAGE_GHOST;
		Nav.HSplitTop(Gap, nullptr, &Nav);
	}
	...
	Nav.HSplitTop(ButtonHeight, &Button, &Nav);
	static CButtonContainer s_CallVoteButton;
	if(DoElysiumNavButton(&s_CallVoteButton, FontIcon::COMMENT, Localize("Call vote"), ActivePage == PAGE_CALLVOTE, &Button))
	{
		NewPage = PAGE_CALLVOTE;
		m_ControlPageOpening = true;
	}

	// bottom group: demos, settings, disconnect
	Nav.HSplitBottom(ButtonHeight, &Nav, &Button);
	static CButtonContainer s_DisconnectButton;
	if(DoElysiumNavButton(&s_DisconnectButton, FontIcon::RIGHT_FROM_BRACKET, Localize("Disconnect"), false, &Button))
	{
	...
	Nav.HSplitBottom(ButtonHeight, &Nav, &Button);
	static CButtonContainer s_SettingsButton;
	if(DoElysiumNavButton(&s_SettingsButton, FontIcon::GEAR, Localize("Settings"), ActivePage == PAGE_SETTINGS, &Button))
		NewPage = PAGE_SETTINGS;
	Nav.HSplitBottom(Gap, &Nav, nullptr);
...
void CMenus::RenderElysiumInGameMenu(CUIRect Screen, IClient::EClientState ClientState)
{
	CUIRect TopBar, Body, StatusBar, Nav, Content;
	...
	RenderElysiumTopBar(TopBar);
	const int NewPage = RenderElysiumInGameNav(Nav);
	RenderElysiumStatusBar(StatusBar);

	// content: reuse the existing in-game page renderers unchanged
	if(m_GamePage == PAGE_GAME)
	{
		...
		RenderServerInfo(Content);
	else if(m_GamePage == PAGE_NETWORK)
		RenderInGameNetwork(Content);
	else if(m_GamePage == PAGE_GHOST)
		RenderGhost(Content);
	else if(m_GamePage == PAGE_CALLVOTE)
		RenderServerControl(Content);
...
void CMenus::Render()
{
	...
	case IClient::STATE_OFFLINE:
		...
		else if(g_Config.m_ClElysiumUi)
		{
			RenderElysiumMainMenu(Screen, ClientState);
		}
		else
		{
	...
	case IClient::STATE_ONLINE:
		...
		{
			RenderElysiumInGameMenu(Screen, ClientState);
		}
		else
		{
			CUIRect TabBar, MainView;
...
void CMenus::UpdateColors()
{
	if(g_Config.m_ClElysiumUi)
	{
		// Elysium UI: consistent dark theme instead of the user's raw UiColor.
		const ElysiumUi::STheme &Theme = ElysiumUi::Theme();
		ms_GuiColor = Theme.m_BgPrimary;

		ms_ColorTabbarInactiveOutgame = Theme.m_BgSecondary.WithMultipliedAlpha(0.55f);
		ms_ColorTabbarActiveOutgame = Theme.m_CardBg;
		ms_ColorTabbarHoverOutgame = Theme.m_Hover;

		ms_ColorTabbarInactiveIngame = Theme.m_BgSecondary.WithMultipliedAlpha(0.80f);
		ms_ColorTabbarActiveIngame = Theme.m_CardBg;
		ms_ColorTabbarHoverIngame = Theme.m_Hover;
		return;
	}
...
void CMenus::RenderElysiumBackground()
{
	const float ScreenHeight = 300.0f;
	const float ScreenWidth = ScreenHeight * Graphics()->ScreenAspect();
	Graphics()->MapScreen(0.0f, 0.0f, ScreenWidth, ScreenHeight);
	...
	// vertical gradient, clearly darker than the interface panels so they stand out
	Graphics()->QuadsBegin();
	const ColorRGBA Base = Theme.m_BgPrimary;
	const ColorRGBA Top = ColorRGBA(Base.r * 0.55f, Base.g * 0.55f, Base.b * 0.55f, 1.0f);
	const ColorRGBA Bottom = ColorRGBA(Base.r * 0.24f, Base.g * 0.24f, Base.b * 0.24f, 1.0f);
	Graphics()->SetColor4(Top, Top, Bottom, Bottom);
	const IGraphics::CQuadItem BgQuad(0, 0, ScreenWidth, ScreenHeight);
	Graphics()->QuadsDrawTL(&BgQuad, 1);
	Graphics()->QuadsEnd();

	// soft accent glow fading down from the top
	Graphics()->QuadsBegin();
	const ColorRGBA Glow = Theme.m_AccentPrimary.WithAlpha(0.10f);
	const ColorRGBA GlowFade = Theme.m_AccentPrimary.WithAlpha(0.0f);
	Graphics()->SetColor4(Glow, Glow, GlowFade, GlowFade);
	const IGraphics::CQuadItem GlowQuad(0, 0, ScreenWidth, ScreenHeight * 0.6f);
	Graphics()->QuadsDrawTL(&GlowQuad, 1);
	Graphics()->QuadsEnd();

	// secondary accent glow rising from the bottom-right corner
	Graphics()->QuadsBegin();
	const ColorRGBA Glow2 = Theme.m_AccentSecondary.WithAlpha(0.0f);
	const ColorRGBA Glow2On = Theme.m_AccentSecondary.WithAlpha(0.06f);
	Graphics()->SetColor4(Glow2, Glow2, Glow2, Glow2On);
	const IGraphics::CQuadItem GlowQuad2(ScreenWidth * 0.4f, ScreenHeight * 0.5f, ScreenWidth * 0.6f, ScreenHeight * 0.5f);
	Graphics()->QuadsDrawTL(&GlowQuad2, 1);
	Graphics()->QuadsEnd();
	...
	const IGraphics::CQuadItem BlobQuadItem(-100, -100, ScreenWidth + 200, ScreenHeight + 200);
	Graphics()->QuadsDrawTL(&BlobQuadItem, 1);
	Graphics()->QuadsEnd();
