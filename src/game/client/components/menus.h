...
protected:
	...
	// Elysium UI main-menu chrome (left nav + top bar + status bar)
	void RenderElysiumMainMenu(CUIRect Screen, IClient::EClientState ClientState);
	void RenderElysiumInGameMenu(CUIRect Screen, IClient::EClientState ClientState);
	int RenderElysiumNav(CUIRect Nav, IClient::EClientState ClientState);
	int RenderElysiumInGameNav(CUIRect Nav);
	void RenderElysiumTopBar(CUIRect Bar);
	void RenderElysiumStatusBar(CUIRect Bar);
	void RenderElysiumBackground();
	int DoElysiumNavButton(class CButtonContainer *pButtonContainer, const char *pIcon, const char *pText, bool Active, const CUIRect *pRect);
...
private:
	...
	// Elysium client (found in menus_settings.cpp)
	void RenderSettingsElysium(CUIRect MainView);
	void RequestElysiumUpdateCheck();
	void ResetElysiumSettings();
};
#endif
