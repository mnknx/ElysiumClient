...
void CMenus::RequestElysiumUpdateCheck()
{
	// The actual update service is intentionally not wired up yet; this is the
	// single entry point that future networking code should hook into.
	dbg_msg("elysium", "Update check requested.");
}

void CMenus::ResetElysiumSettings()
{
	// Only Elysium-specific settings are reset here; DDNet settings are untouched.
	g_Config.m_ClElysiumEnabled = 1;
	g_Config.m_ClElysiumShowLogo = 1;
	g_Config.m_ClElysiumShowNotifications = 1;
	g_Config.m_ClElysiumShowServerTags = 1;
	g_Config.m_ClElysiumHighlightServers = 1;
	g_Config.m_ClElysiumOnlyServers = 0;
	g_Config.m_ClElysiumAutoServerFilter = 0;
	g_Config.m_ClElysiumConnectionNotifications = 1;
	g_Config.m_ClElysiumUpdateNotifications = 1;
	g_Config.m_ClElysiumSystemNotifications = 1;
	g_Config.m_ClElysiumCheckUpdates = 1;
}
...
void CMenus::RenderSettingsElysium(CUIRect MainView)
{
	...
	// Local helpers so the layout stays consistent (margins/padding/spacing)
	// and every element is registered with the scroll region.
	auto &&AddRect = [&](float Height) -> CUIRect {
		CUIRect Row;
		MainView.HSplitTop(Height, &Row, &MainView);
		s_ScrollRegion.AddRect(Row);
		return Row;
	};
	auto &&DoHeadline = [&](const char *pLabel) {
		CUIRect Headline = AddRect(HeadlineHeight);
		Ui()->DoLabel(&Headline, pLabel, HeadlineFontSize, TEXTALIGN_ML);
	};
	auto &&DoCheckBox = [&](int *pConfig, const char *pLabel) {
		CUIRect Row = AddRect(LineHeight);
		if(DoButton_CheckBox(pConfig, pLabel, *pConfig, &Row))
			*pConfig ^= 1;
		AddRect(LineSpacing);
	};

	// General
	DoHeadline(Localize("General"));
	DoCheckBox(&g_Config.m_ClElysiumEnabled, Localize("Enable Elysium features"));
	AddRect(SectionSpacing);

	// User Interface (Elysium UI theme)
	DoHeadline(Localize("User Interface"));
	DoCheckBox(&g_Config.m_ClElysiumUi, Localize("Enable Elysium UI"));
	{
		CUIRect ScaleRow = AddRect(LineHeight);
		Ui()->DoScrollbarOption(&g_Config.m_ClElysiumUiScale, &g_Config.m_ClElysiumUiScale, &ScaleRow, Localize("UI scale"), 50, 200, &CUi::ms_LinearScrollbarScale, 0u, "%");
		AddRect(LineSpacing);
	}
	DoCheckBox(&g_Config.m_ClElysiumAnimations, Localize("Enable animations"));
	DoCheckBox(&g_Config.m_ClElysiumReduceAnimations, Localize("Reduce animations"));
	DoCheckBox(&g_Config.m_ClElysiumTransparentPanels, Localize("Transparent panels"));
	DoCheckBox(&g_Config.m_ClElysiumHighContrast, Localize("High contrast mode"));
	DoCheckBox(&g_Config.m_ClElysiumCompactServerList, Localize("Compact server list"));
	DoCheckBox(&g_Config.m_ClElysiumCompactChat, Localize("Compact chat"));
	DoCheckBox(&g_Config.m_ClElysiumTooltips, Localize("Show tooltips"));
	AddRect(SectionSpacing);

	// Interface
	DoHeadline(Localize("Interface"));
	DoCheckBox(&g_Config.m_ClElysiumShowLogo, Localize("Show Elysium logo"));
	DoCheckBox(&g_Config.m_ClElysiumShowNotifications, Localize("Show Elysium notifications"));
	DoCheckBox(&g_Config.m_ClElysiumShowServerTags, Localize("Show Elysium server tags"));
	AddRect(SectionSpacing);

	// Server Browser
	DoHeadline(Localize("Server Browser"));
	DoCheckBox(&g_Config.m_ClElysiumHighlightServers, Localize("Highlight Elysium servers"));
	DoCheckBox(&g_Config.m_ClElysiumOnlyServers, Localize("Show only Elysium servers"));
	DoCheckBox(&g_Config.m_ClElysiumAutoServerFilter, Localize("Apply Elysium server filter automatically"));
	AddRect(SectionSpacing);

	// Notifications
	DoHeadline(Localize("Notifications"));
	DoCheckBox(&g_Config.m_ClElysiumConnectionNotifications, Localize("Show connection notifications"));
	DoCheckBox(&g_Config.m_ClElysiumUpdateNotifications, Localize("Show update notifications"));
	DoCheckBox(&g_Config.m_ClElysiumSystemNotifications, Localize("Show system notifications"));
	AddRect(SectionSpacing);

	// Updates
	DoHeadline(Localize("Updates"));
	DoCheckBox(&g_Config.m_ClElysiumCheckUpdates, Localize("Check for updates on startup"));

	CUIRect VersionRow = AddRect(LineHeight);
	TextRender()->TextColor(0.7f, 0.7f, 0.7f, 1.0f);
	Ui()->DoLabel(&VersionRow, Localize("Client version: Development Build"), 14.0f, TEXTALIGN_ML);
	TextRender()->TextColor(1.0f, 1.0f, 1.0f, 1.0f);
	AddRect(LineSpacing);

	CUIRect UpdateButtonRow = AddRect(LineHeight);
	CUIRect UpdateButton;
	UpdateButtonRow.VSplitLeft(160.0f, &UpdateButton, nullptr);
	static CButtonContainer s_UpdateButton;
	if(DoButton_Menu(&s_UpdateButton, Localize("Check for Updates"), 0, &UpdateButton))
		RequestElysiumUpdateCheck();
	AddRect(SectionSpacing);

	// Reset
	CUIRect ResetButtonRow = AddRect(LineHeight);
	CUIRect ResetButton;
	ResetButtonRow.VSplitLeft(160.0f, &ResetButton, nullptr);
	static CButtonContainer s_ResetButton;
	if(DoButton_Menu(&s_ResetButton, Localize("Reset Elysium Settings"), 0, &ResetButton))
	{
		PopupConfirm(Localize("Reset Elysium Settings"),
			Localize("Are you sure you want to reset the Elysium settings to their default values?"),
			Localize("Reset"), Localize("Cancel"), &CMenus::ResetElysiumSettings);
	}
	AddRect(LineSpacing);
